            ...
            foreach ($user_orders as $order) {
                ...
                <tr >
                    <td class="dokan-order-id" data-title="<?php _e( 'Order', 'dokan' ); ?>" >
                        <?php echo '<a href="' . wp_nonce_url( add_query_arg( array( 'order_id' => $the_order->get_id() ), dokan_get_navigation_url( 'orders' ) ), 'dokan_view_order' ) . '"><strong>' . sprintf( __( 'Order %s', 'dokan' ), esc_attr( $the_order->get_order_number() ) ) . '</strong></a>'; ?>
                    </td>
                    ...
                    <td class="dokan-order-date" data-title="<?php _e( 'Date', 'dokan' ); ?>" >
                        ...
                        if ( '0000-00-00 00:00:00' == $the_order->order_date ) {
                            ...
                            $t_time = get_the_time( __( 'Y/m/d g:i:s A', 'dokan' ), $the_order->get_id() );
                            ...
                            if ( $time_diff > 0 && $time_diff < 24 * 60 * 60 )
                                ...
                                $h_time = get_the_time( __( 'Y/m/d', 'dokan' ), $the_order->get_id() );
                        }
