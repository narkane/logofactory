 ...
 * Base class
 */
if ( !class_exists( 'Order_Delivery_Note' ) ) {
	/**
	 ...
	 */
	final class Order_Delivery_Note {
		...
		public function init_hooks() {
			add_action( 'init', array( $this, 'localise' ),9999 );
			add_action( 'init', array( $this, 'load' ),9999 );
		...
		public function include_classes() {
			include_once( 'nbt_delivery_note_print.php' );
			require_once NBT_ODN_PATH . 'inc/settings.php';
			include_once( 'nbt_delivery_note_writepanel.php' );
			include_once( 'nbt_delivery_note_theme.php' );
		...
		// /	die;
			include_once( 'nbt_delivery_note_functions.php' );
			include_once( 'nbt_delivery_note_hooks.php' );
		}
		...
		public function load() {
			...
			if ( $this->is_woocommerce_activated() ) {	
				...
				$this->print = new NBT_Order_Delivery_Note_Print();
				$this->settings= new NBT_Order_Delivery_Note_Settings();
				$this->writepanel = new NBT_Order_Delivery_Note_Writepanel();
				$this->theme = new NBT_Order_Delivery_Note_Theme();
				...
				add_action( 'wp_loaded', array( $this, 'include_template_functions' ),9999 );
        ...
        public static function wcdn_ts_add_plugin_tracking_data ( $data ) {
            if ( isset( $_GET[ 'wcdn_tracker_optin' ] ) && isset( $_GET[ 'wcdn_tracker_nonce' ] ) && wp_verify_nonce( $_GET[ 'wcdn_tracker_nonce' ], 'wcdn_tracker_optin' ) ) {
				...
				$plugin_data[ 'print_in_myaccount' ]      = NB_Solution::get_setting('order-delivery-note')['wc_order_delivery_noteprint_my_account'];
				$plugin_data[ 'print_in_vieworder' ]      = NB_Solution::get_setting('order-delivery-note')['wc_order_delivery_noteprint_my_account'];
				$plugin_data[ 'print_in_email' ]          = NB_Solution::get_setting('order-delivery-note')['wc_order_delivery_noteprint_my_account'];
		...
		public function update() {
			...
			if( version_compare( $option_version, self::$plugin_version, '<' ) ) {
				...
				if( version_compare( $option_version, '4.2.0', '<' ) ) {
					...
					$data = NB_Solution::get_setting('order-delivery-note');
					$invoice_start = intval( get_option( 'wcdn_invoice_number_start', 1 ) );
					$invoice_counter = isset($data['wc_order_delivery_note_next_number']) ? intval( $data['wc_order_delivery_note_next_number'] ) : 0;
					update_option( 'wc_order_delivery_note_next_number', $invoice_start + $invoice_counter );	
 ...
 */
function WCDN() {
	return Order_Delivery_Note::instance();
}
