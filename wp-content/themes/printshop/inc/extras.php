...
function printshop_sidebar_desc($sidebar_id ) {
	...
	if ( $column > $footer_columns ) {
		$desc = esc_html__( 'This widget area is currently disabled. You can enable it Theme Options - Footer section.', 'wpnetbase' );
	}
...
function printshop_topbar_desc($sidebar_id ) {
	...
	if ( $header_style == '' || $header_style == 'header-default' ) {
		$desc = esc_html__( 'This widget area is currently disabled because you are using default header ( Theme Option > Header ) and it only available for Header Topbar or Header Centered.', 'wpnetbase' );
	} else {
		$desc = '';
	}
