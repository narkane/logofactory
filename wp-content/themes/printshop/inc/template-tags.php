...
function printshop_paging_nav() {
	...
	$links = paginate_links( array(
		'wpnetbase'     => $pagenum_link,
		'format'    => $format,
		'total'     => $GLOBALS['wp_query']->max_num_pages,
		'current'   => $paged,
		'mid_size'  => 1,
		'add_args'  => array_map( 'urlencode', $query_args ),
		'prev_text' =>  wp_kses(__('<i class=\'fa fa-chevron-left\'></i>', 'wpnetbase' ), array('i' => array('class' => array()))),
		'next_text' => wp_kses(__( '<i class=\'fa fa-chevron-right\'></i>', 'wpnetbase' ), array('i' => array('class' => array()))),
	) );
	...
	<nav class="navigation paging-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php esc_html_e( 'Posts navigation', 'wpnetbase' ); ?></h1>
...
function printshop_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'wpnetbase' ); ?></h1>
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', _x( '<span class="meta-nav">&</span>&nbsp;%title', 'Previous post link', 'wpnetbase' ) );
				next_post_link(     '<div class="nav-next">%link</div>',     _x( '%title&nbsp;<span class="meta-nav">&rarr;</span>', 'Next post link',     'wpnetbase' ) );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
function printshop_entry_footer() {
	...
	if ( $category_list ) {
		$meta_text .= wp_kses(__( '<i class="fa fa-file"></i> ', 'wpnetbase' ), array('i' => array('class' => array()))) . '%1$s';
	}
	if ( $tag_list ) {
		$meta_text .= wp_kses(__( '<i class="fa fa-tag"></i> ', 'wpnetbase' ), array('i' =>array('class' => array()))) . '%2$s';
	}
	printf(
		$meta_text,
		$category_list,
	...
	<div class="entry-author clearfix">
		...
		<div class="entry-author-byline">
			<?php
			printf(
				_x( 'Written by %s', 'author byline', 'wpnetbase' ),
				sprintf(
					'<a class="vcard" href="%1$s">%2$s</a>',
					esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
...
function printshop_comment($comment, $args, $depth ) {
    ...
    <li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
        <p><?php esc_html_e( 'Pingback:', 'wpnetbase' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'wpnetbase' ), '<span class="edit-link">', '</span>' ); ?></p>
    <?php
            break;
        default :
                ...
                <header class="comment-meta comment-author vcard">
                    <?php
                        printf( '<cite><b class="fn">%1$s</b> %2$s</cite>',
                            ...
                            ( $comment->user_id === $post->post_author ) ? '<span>' . esc_html__( 'Post author', 'wpnetbase' ) . '</span>' : ''
                        );
                        printf( '<a class="comment-time" href="%1$s"><time datetime="%2$s">%3$s</time></a>',
                            esc_url( get_comment_link( $comment->comment_ID ) ),
                            get_comment_time( 'c' ),
                            /* translators: 1: date, 2: time */
                            sprintf( esc_html__( '%1$s', 'wpnetbase' ), get_comment_date() )
                        );
                        comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'wpnetbase' ), 'after' => '', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) );
                        edit_comment_link( esc_html__( 'Edit', 'wpnetbase' ), '<span class="edit-link">', '</span>' );
                    ?>
                </header><!-- .comment-meta -->

                <?php if ( '0' == $comment->comment_approved ) : ?>
                    <p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'wpnetbase' ); ?></p>
                <?php endif; ?>
...
function printshop_breadcrumb() {
	if( function_exists('bcn_display') ) {
		if ( is_front_page() && is_home() ) {
			...
			?>
			<div class="breadcrumbs">			
				<span><?php echo esc_html__('You are here:', 'wpnetbase');?></span>
				<?php bcn_display(); ?>				
			</div>
			<?php
		}
...
function printshop_list_child_pages($pageID, $order, $orderby, $exclude, $layout, $column, $number, $readmore_text ) {
	...
	if ( $readmore_text == '' ) {
		$readmore_text = esc_html__('Read More', 'wpnetbase');
	}
	...
	if ( $page_childrens->have_posts() ) :
		...
		$output .= '
			...
			$output .= esc_html__( 'Sorry, there is no child pages under your selected page.', 'wpnetbase' );
	endif;
