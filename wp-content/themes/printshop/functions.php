...
function printshop_setup() {
	...
	load_theme_textdomain( 'wpnetbase', $language_folder );
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary', 'wpnetbase' ),
		'footer' => esc_html__( 'Footer', 'wpnetbase' ),
		) );
...
function printshop_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Blog Sidebar', 'wpnetbase' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__('Sidebar in the blog pages', 'wpnetbase'),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Top Language', 'wpnetbase' ),
		'id'            => 'language-1',
		'description'   => esc_html__('Widget area for language change in header', 'wpnetbase'),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Top Currency', 'wpnetbase' ),
		'id'            => 'currency-1',
		'description'   => esc_html__('Widget area for currency change in header', 'wpnetbase'),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h4 class="widget-title">',
		'after_title'   => '</h4>',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Cart Header', 'wpnetbase' ),
		'id'            => 'cart-header',
		'description'   => esc_html__('Cart Header', 'wpnetbase'),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h4 class="widget-title">',
		'after_title'   => '</h4>',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Page Sidebar', 'wpnetbase' ),
		'id'            => 'sidebar-2',
		'description'   => esc_html__('Sidebar for pages', 'wpnetbase'),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h2 class="widget-title">',
		'after_title'   => '</h2>',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'WooCommerce Sidebar', 'wpnetbase' ),
		'id'            => 'sidebar-woo',
		'description'   => esc_html__('Sidebar for WooCommerce pages', 'wpnetbase'),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h2 class="widget-title">',
		'after_title'   => '</h2>',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Footer 1', 'wpnetbase' ),
		'id'            => 'footer-1',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Footer 2', 'wpnetbase' ),
		'id'            => 'footer-2',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Footer 3', 'wpnetbase' ),
		'id'            => 'footer-3',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Footer 4', 'wpnetbase' ),
		'id'            => 'footer-4',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Top Bar Left', 'wpnetbase' ),
		'id'            => 'topbar-left',
		'description'   => esc_html__('Widget area for top bar left', 'wpnetbase'),
		'before_widget' => '<aside class="topbar-widget widget %2$s">',
		'after_widget'  => '</aside>',

		) );
	register_sidebar( array(
		'name'          => esc_html__( 'Top Bar Right', 'wpnetbase' ),
		'id'            => 'topbar-right',
		'description'   => esc_html__('Widget area for top bar right', 'wpnetbase'),
		'before_widget' => '<aside class="topbar-widget widget %2$s">',
		'after_widget'  => '</aside>',
		...
		register_sidebar( array(
		'name'          => esc_html__( 'Menu Header Creative', 'wpnetbase' ),
		'id'            => 'menu-header-creative',
		'description'   => esc_html__('Widget area for menu header creative', 'wpnetbase'),
		'before_widget' => '<div>',
		'after_widget'  => '</div>',
		'before_title'  => '<h2 class="widget-title">',
		'after_title'   => '</h2>',
		...
		register_sidebar( array(
		'name'          => esc_html__( 'Footer bottom', 'wpnetbase' ),
		'id'            => 'footer-bottom',
...
function printshop_scripts() {
	...
	wp_enqueue_script( 'printshop-theme', get_template_directory_uri() . '/assets/js/theme.js', array('jquery', 'owl.carousel'), '', true );
	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}

	
	wp_localize_script('jquery', 'nb_printshop', array(
        'label' => array(
            'wishlist' => __('Wishlist', 'wpnetbase'),
            'quickview' => __('Quickview', 'wpnetbase'),
            'browse_wishlist' => __('Browse Wishlist', 'wpnetbase'),
            'start_desgin' => __('Start Design', 'wpnetbase'),
            'compare' => __('Compare', 'wpnetbase'),
        )
    ));
}
		...
		if( $neatly_posts->have_posts() ) {
			...
			<li>
				...
				<div class="post-thumb-info">
					...
					?>
					<a class="post-info-readmore" href="<?php the_permalink(); ?>"><?php esc_html_e('Read More', 'wpnetbase')?><i class="fa fa-chevron-right" ></i></a>
				</div>
			</li>
			<?php endwhile;
			echo '</ul>';
...
public function form( $instance ) {
	$title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'Recent Post', 'wpnetbase' );
	if (isset( $instance['number' ] ) ) {
		$number = $instance['number'];
	} else { $number = '5'; }
	?>
	<p>
		<label for="<?php echo esc_html($this->get_field_id('title')); ?>"><?php esc_html__('Title:', 'wpnetbase'); ?></label>
		<input class="widefat" id="<?php echo esc_html($this->get_field_id('title')); ?>" name="<?php echo esc_html($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
	</p>
	<p><em><?php esc_html_e('Use the following options to customize the display.', 'wpnetbase'); ?></em></p>
	<p style="border-bottom:4px double #eee;padding: 0 0 10px;">
		<label for="<?php echo esc_html($this->get_field_id( 'number' )); ?>"><?php esc_html_e('Number of posts:', 'wpnetbase');?></label>
...
if ( in_array( 'siteorigin-panels/siteorigin-panels.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) )
{
	...
	function printshop_custom_row_style_fields($fields) {
		$fields['nbparallax'] = array(
			'name'        => __('Wpnetbase Parallax', 'wpnetbase'),
			'type'        => 'checkbox',
			'group'       => 'design',
			'description' => __('If enabled, the background image will have a parallax effect.custom by netbaseteam', 'wpnetbase'),
			'priority'    => 8,
		);
...
function bamboo_request($query_string )
{
    ...
    if( isset( $query_string['paged'] ) ) {
    	...
    	if(isset($query_string[$dokan_general['custom_store_url']]) ){
