...
jQuery(document).ready( function() {
    ...
    jQuery.extend( jQuery.datepicker, { afterShow: function( event ) {
        ...
        if ( jQuery( "#nbtodd_number_of_months" ).val() == "1" ) {
    ...
    jQuery( "#e_deliverydate" ).val("").datepicker( { 
    	dateFormat: jQuery( "#nbtodd_delivery_date_format" ).val(), 
    ...
    if ( jQuery( "#nbtodd_field_note" ).val() != '' ) {
        jQuery( "#e_deliverydate_field" ).append( "<small class='nbtodd_field_note'>" + jQuery( "#nbtodd_field_note" ).val() + "</small>" );
    }
...
function orddd_on_select_date( date, inst ) {
	...
	var data = {
        ...
        h_deliverydate: all,
        action: "nbtodd_update_delivery_session"
    };
 ...
 */
function load_lite_functions() {
	if( jQuery( "#nbtodd_auto_populate_first_available_date" ).val() == "on" ) {
		nbtodd_autofil_date_time();
    }
 ...
 *
 * @function nbtodd_autofil_date_time
 * @memberof nbtodd_initialize
 * @since 2.8
 */
function nbtodd_autofil_date_time() {
	var current_date = jQuery( "#nbtodd_current_day" ).val();
	...
	var delay_date = jQuery( "#nbtodd_minimumOrderDays" ).val();
	if( delay_date != "" ) {
...
function minimum_date_to_set( delay_days ) {
	var disabledDays = eval( "[" + jQuery( "#nbtodd_holidays" ).val() + "]" );
	var holidays = [];
	...
	var bookedDays = eval( "[" + jQuery( "#nbtodd_lockout_days" ).val() + "]" );
	...
	for ( j = current_weekday ; current_time <= delay_time ; j++ ) {
		if( j >= 0 ) {
			if ( jQuery( "#nbtodd_calculate_min_time_disabled_days" ).val() != 'on' ) {
				day = 'nbtodd_weekday_' + current_weekday;
				...
				day = 'nbtodd_weekday_' + delay_days.getDay();
				day_check = jQuery( "#" + day ).val();
				if ( day_check == '' ) {
					delay_days.setDate( delay_days.getDate()+1 );
					delay_time = delay_days.getTime();					
...
function avd( date ) {
	...
	var noOfDaysToFind = parseInt( jQuery( "#nbtodd_number_of_dates" ).val() );
	...
	for( var i = 1; i <= loopCounter; i++ ) {
		...
		var day = "";
		day = 'nbtodd_weekday_' + new_date.getDay();
		day_check = jQuery( "#" + day ).val();
	...
	var number_of_months = parseInt( jQuery( "#nbtodd_number_of_months" ).val() );
