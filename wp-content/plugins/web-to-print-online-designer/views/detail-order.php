...
<div id="nbdesign-order-tabs">
    <h2>
        ...
        <ul class="nbd-nav-tab">
            ...
            <li><a href="#convert-download"><?php _e('Download design', 'web-to-print-online-designer') ?></a></li>
        </ul>   
        ...
        <div id="convert-download">
            <div id="nbd-convert-section">
                <h3><?php _e('Download design', 'web-to-print-online-designer'); ?></h3>                
                <div class="nbd-convert-con">
                    <p class="nbd-convert-con-header"><?php _e('SVG', 'web-to-print-online-designer'); ?></p>
                    <div class="nbd-convert-con-inner">
                        <a href="<?php echo add_query_arg(array('download-type' => 'svg', 'nbd_item_key' => $_GET['nbd_item_key'], 'order_id' => $_GET['order_id'], 'product_id' => $_GET['product_id'], 'variation_id' => $_GET['variation_id']), admin_url('admin.php?page=nbdesigner_detail_order')); ?>" class="button-primary" id="download-svg"><?php _e('Download', 'web-to-print-online-designer'); ?></a>
                    </div>
                </div>
                <div class="nbd-convert-con">
                    <div id="conver-png-to-jpg" >
                        <p class="nbd-convert-con-header"><?php _e('PNG', 'web-to-print-online-designer'); ?> <?php if( is_available_imagick() ) _e('( High quantity images )', 'web-to-print-online-designer'); ?></p>
                        <div class="nbd-convert-con-inner">
                            <?php  if( !is_available_imagick() ): ?>
                            <a href="<?php echo add_query_arg(array('download-type' => 'png', 'nbd_item_key' => $_GET['nbd_item_key'], 'order_id' => $_GET['order_id'], 'product_id' => $_GET['product_id'], 'variation_id' => $_GET['variation_id']), admin_url('admin.php?page=nbdesigner_detail_order')); ?>" class="button-primary" id="download-png"><?php _e('Download PNG', 'web-to-print-online-designer'); ?></a>
                            <?php else: ?>
                            <a class="button-primary <?php  if( !is_available_imagick() ) echo 'nbd-unavailable'; ?>" href="<?php echo add_query_arg(array('download-type' => 'png-hires', 'nbd_item_key' => $_GET['nbd_item_key'], 'order_id' => $_GET['order_id'], 'product_id' => $_GET['product_id'], 'variation_id' => $_GET['variation_id']), admin_url('admin.php?page=nbdesigner_detail_order')); ?>" class="button-primary" id="download-png"><?php _e('Download', 'web-to-print-online-designer'); ?></a>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                <div class="nbd-convert-con <?php  if( !is_available_imagick() ) echo 'nbd-unavailable'; ?>" >
                    <p class="nbd-convert-con-header"><?php _e('JPG ( CMYK with ICC profile )', 'web-to-print-online-designer'); ?></p>
                    <div class="nbd-convert-con-inner">
                        <span class="nbd-unavailable-note <?php  if( is_available_imagick() ) echo 'nbd-disabled'; ?>">*</span>
                        <a class="button-primary" href="<?php echo add_query_arg(array('download-type' => 'jpg-hires', 'nbd_item_key' => $_GET['nbd_item_key'], 'order_id' => $_GET['order_id'], 'product_id' => $_GET['product_id'], 'variation_id' => $_GET['variation_id']), admin_url('admin.php?page=nbdesigner_detail_order')); ?>" class="button-primary" id="download-png"><?php _e('Download', 'web-to-print-online-designer'); ?></a>
                        <p><?php _e('Change ICC profile', 'web-to-print-online-designer'); ?> <a target="_blank" href="<?php echo esc_url(admin_url('admin.php?page=nbdesigner&tab=output#nbdesigner_default_icc_profile')); ?>"><?php _e('here', 'web-to-print-online-designer'); ?></a></p>
                    </div>
                </div> 
                <?php if(nbd_check_license()): ?>
                <div class="nbd-convert-con">
                    <div id="download-pdf" >
                        <p class="nbd-convert-con-header"><?php _e('PDF', 'web-to-print-online-designer'); ?></p>
                        <div class="nbd-convert-con-inner">
                            <a href="<?php echo add_query_arg(array('download-type' => 'pdf', 'nbd_item_key' => $_GET['nbd_item_key'], 'order_id' => $_GET['order_id'], 'product_id' => $_GET['product_id'], 'variation_id' => $_GET['variation_id']), admin_url('admin.php?page=nbdesigner_detail_order')); ?>" class="button-primary" id="download-svg"><?php _e('Download', 'web-to-print-online-designer'); ?></a>
                            <p><a href="javascript:void(0)" onclick="activeTabPdf()"><?php _e('Export PDF with config', 'web-to-print-online-designer'); ?></a></p>
                        </div>
                    </div>
                </div>
                <?php endif; ?>
            ...
            <table class="form-table">
                <tbody>
                    ...
                    <tr valign="top">
                        ...
                        <td class="forminp forminp-text">
                            <?php _e('Width', 'web-to-print-online-designer'); ?>: <input name="pdf[<?php echo $key; ?>][product-width]" type="text" readonly style="width: 60px;" value="<?php echo ($proWidth * $unitRatio); ?>"> 
                            <?php _e('Hight', 'web-to-print-online-designer'); ?>: <input name="pdf[<?php echo $key; ?>][product-height]" type="text" readonly style="width: 60px;" value="<?php echo ($proHeight * $unitRatio); ?>"> 
                            <small><?php _e('In mm', 'web-to-print-online-designer'); ?></small> <br /> ( <?php echo $proWidth . '&times;' . $proHeight . ' ' . $unit; ?> )
                        </td>                        
                    </tr>
                    <tr valign="top">
                        <th scope="row" class="titledesc">
                    ...
                    <tr valign="top">
                        ...
                        <td class="forminp forminp-text">
                            <p>
<!--                                <input name="pdf[<?php //echo $key; ?>][show-bleed-line]" type="radio" value="yes" onclick="showSettingBleed(this)">-->
                                <input name="pdf[<?php echo $key; ?>][show-bleed-line]" type="radio" value="yes">
                                    <?php _e('Yes', 'web-to-print-online-designer'); ?>
                            </p>   
                            <p>
<!--                                <input name="pdf[<?php //echo $key; ?>][show-bleed-line]" type="radio" value="no" checked="checked" onclick="showSettingBleed(this)">-->
                                <input name="pdf[<?php echo $key; ?>][show-bleed-line]" type="radio" value="no" checked="checked">
                                    <?php _e('No', 'web-to-print-online-designer'); ?>
                            </p>
                        </td>
                    </tr>   
                    ...
                    <tr valign="top">
                        <th scope="row" class="titledesc">
                            <label><?php _e('Include background image', 'web-to-print-online-designer'); ?></label>
                        </th>
                        <td class="forminp forminp-text">
                            ...
                            &nbsp;<small><?php _e('Include background image if background type is image.', 'web-to-print-online-designer'); ?></small>
                        </td>
                    </tr>
                </tbody>
            </table>
